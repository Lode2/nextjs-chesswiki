...
export default function Chessboard(props) {
    ...
    const imageList = Array.from(Array(64).keys()).map(item => {
        return <Image src="/images/bb.png" width={45} height={45} />
    })
    ...
    const renderedOutput = Array.from(Array(64).keys()).map(item => {
        ...
        return <div key={item.toString()} onMouseEnter={myFunction} style={{ position: 'relative', backgroundColor: squareColor[mod2], width: boardSize / 8, height: boardSize / 8 }}>{imageList[item]}<div key={squareId} style={{ position: 'absolute', left: 0, bottom: 2, fontSize: 10, textAlign: 'center', userSelect: 'none', width: boardSize / 32, height: boardSize / 32 }}>{squareId}</div> </div >
    })
    function myFunction() {
